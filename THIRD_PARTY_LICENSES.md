...
This document inventories the direct runtime and build dependencies of the
dashboard shipped from `surfaces/dashboard`; the exact resolved versions are
recorded in [`bun.lock`](./bun.lock).

Each project remains under its own license. The links in the table point to the
upstream project for that package; the canonical license texts are listed below.
...
| `tw-animate-css` | `^1.4.0` | MIT | [tw-animate-css](https://github.com/Wombosvideo/tw-animate-css) |
...
The canonical texts for the licenses used by these dependencies are available
from the following project-maintained sources:
