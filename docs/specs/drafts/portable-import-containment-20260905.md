...
Independent reproduction command (run from this worktree, with no inherited credentials) was the Bun inline probe importing `Database`, `ensureUnifiedSchema`, `runMigrations`, and `registerPortableCommands` from this checkout; it created `/tmp/signet-import-containment-current/{source,target}`, inserted `memory-alpha`/`memory-beta` plus one artifact, ran `export --json` then `import --json`, and queried the target. Observed output: export `2 memories`; import `2 memories imported`; target `after: []`, artifact count `0`; bundle fields excluded `agent_id`/`visibility`, and `identity/DREAMING.md` was absent. This reproduces the historical E3 result described by the reconsideration draft without depending on files absent from this branch.
...
Contain the unsafe path while owner migration is pending. At the CLI boundary, read the bundle without writing, then reject malformed top-level JSON, unknown or duplicate paths, malformed JSONL, missing manifest version/custody scope, missing required fields, unsupported versions, and records whose scope cannot be established. Validate every record and path before touching identity files, configuration, directories, or databases. Reject an old narrow bundle with an actionable recovery message; never invent `default` scope.

The immediate containment is smaller: make the current CLI `import <path>` exit nonzero with the exact error `Portable import is temporarily unsupported: this bundle lacks authoritative scope and source custody. Preserve the bundle and target; use a complete daemon restore when available.` before opening `AGENTS_DIR/memory/memories.db`, creating `memory/`, or writing any identity/config/skill file. Do not add a validator or accepted import path to this containment change. Keep export available, but describe its output as a limited, inspectable export rather than a complete backup.
...
## Format, migration, and rollback decisions
...
Preserve the original bundle byte-for-byte and the existing target. Stage and validate a fresh generation before activation. On rejection or staging failure, discard only staging and prove source/target bytes and durable counts are unchanged. Rollback is a generation/pointer operation under the owner lease; after new writes, export/reconcile or repair forward. Never mutate the source or silently select another workspace.
...
- any existing bundle path, including malformed JSON, malformed JSONL, omitted scope, symlink/out-of-root content, and conflict options, returns the exact unsupported error before opening the target DB or writing any target file;
...
- the command does not report imported/restored counts, leaves no temporary target directory or DB handle, and documents export as limited.
...
Compatibility consequence: `signet import` is intentionally unavailable for current and legacy narrow bundles until D lands the daemon-owned restore contract. Existing bundles remain preserved for later inspection/translation; no supported essential runtime caller was found for the core import helpers, so no public helper removal is proposed now. Complete custody remains conditional on the table/asset inventory and owner snapshot proof; this brief does not invent precision for that larger migration.
