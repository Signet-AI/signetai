...
| `explicit-recall-surface-alignment` | approved | `docs/specs/approved/explicit-recall-surface-alignment.md` | `memory-pipeline-v2`, `signet-runtime` | - | Consolidate TypeScript explicit recall around `hybridRecall()` and `/api/memory/recall`, keep prompt-submit lightweight with a conservative low-signal gate, and stop CLI/MCP/connector consumers from flattening away useful metadata |
| `memory-content-safety` | approved | `docs/specs/approved/memory-content-safety.md` | `memory-pipeline-v2`, `explicit-recall-surface-alignment`, `native-harness-memory-bridge`, `dreaming-memory-consolidation` | - | Scan untrusted remembered content and source projections before prompt-facing recall, reranking, native fallback, Dreaming, and MEMORY.md use while preserving immutable evidence and provenance. |
...
| `kimi-harness-provider-support` | approved | `docs/specs/approved/kimi-harness-provider-support.md` | `signet-runtime`, `model-provider-router` | - | Phase 1 connector, setup detection, lifecycle hooks, and ACPX agent registration. No separate Kimi provider runtime. |
| `native-harness-memory-bridge` | approved | `docs/specs/approved/native-harness-memory-bridge.md` | `memory-md-rolling-window-lineage`, `signet-runtime` | - | Indexes native harness memory artifacts into Signet recall without materializing duplicate Signet-authored memory rows; Codex, Claude Code, and Hermes Agent use the shared native bridge with soft-deleted stale artifacts excluded from active recall. |
| `cache-stable-memory-injection` | approved | `docs/specs/approved/cache-stable-memory-injection.md` | `lossless-working-memory-closure`, `signet-runtime` | - | Splits deterministic session-start instructions from dynamic context, keeps provider-bound copies out of canonical transcripts, and documents harness-specific degraded modes and measurements. |
