...
| `lossless-working-memory-closure` | approved | `docs/specs/approved/lossless-working-memory-closure.md` | `memory-md-temporal-head`, `lossless-working-memory-runtime`, `signet-runtime` | - | Hardens the no-gap runtime contract with strict three-tier behavior (global head, thread heads, lossless lineage), merge-safe updates, compaction parity, transcript fallback discipline, and explicit harness fidelity docs for generalized multi-thread use. |
