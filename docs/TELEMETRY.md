...
| `recall.performed` | every completed shared recall search | `surface`, `type` (`semantic` / `keyword` / `temporal` / `graph`), `results`, `latencyMs`, `truncated`, `partial` (`true` when FTS coverage is incomplete), optional `degradation` (`fts_incomplete`) |
