...
- `pipeline/maintenance-worker.ts:347` (withReadDbAsync)
...
- `db:repair.embedding-gap-stats.read` (withReadDbAsync)
- `db:repair.embedding-migration-stats.read` (withReadDbAsync)
- `db:repair.orphaned-embedding-stats.read` (withReadDbAsync)
...
- `db:queue-diagnostics.queue.read` (withReadDb)
