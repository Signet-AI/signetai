...
| `epistemic_assertions` | 071, 132 | CLI/API | source-attributed assertion ledger | Records who claimed, believed, observed, decided, preferred, denied, or questioned something, with confidence, evidence, source provenance, status, and optional link to an applied claim attribute. Migration 132 adds an observer-scoped query index; the agent scope is the agent-only observer projection. Assertions do not by themselves make the statement current ontology truth. |
...
| `dreaming_state`, `dreaming_passes` | 055 | CLI/API via dream status/trigger | dreaming worker | Pass IDs identify the same pass-creation attempt across owner retries and reconciliation. Dream promotion is apply-first with provenance; generated or ambiguous candidates remain preview/questions until explicitly applied. |
...
| `GET /api/ontology/assertions` | CLI/API | read-only | Lists source-attributed assertions by observer scope (the authorized `agent_id`), entity, predicate, speaker, source, status, or text query. `observer_id` is an explicit alias for the agent-only observer projection and must match `agent_id`; arbitrary peer observers are not enabled. Covered by assertion tests. |
| `GET /api/ontology/assertions/:id` | CLI/API | read-only | Reads one same-agent, observer-scoped assertion. `observer_id`, when supplied, must match the authorized `agent_id`; cross-agent ids return 404. Covered by assertion tests. |
...
| `POST /api/ontology/assertions` | CLI/API | creates source-attributed assertion | Requires evidence or source provenance. The optional `observer_id` must equal the authorized `agent_id`; `speaker` remains attribution, not observer scope. Semantic `ontology_claim` premises use the existing same-agent provenance gate. Covered by assertion tests. |
...
- `signet ontology assertions` | CLI | read-only | Lists source-attributed assertions with filters for observer scope, entity, predicate, speaker, source, status, query, and agent. `--observer` is an explicit agent-only observer projection and must match `--agent`. |
