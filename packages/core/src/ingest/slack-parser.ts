...
function loadUsers(dirPath: string): Map<string, string> {
	...
	try {
		const raw = JSON.parse(readFileSync(usersPath, "utf-8")) as SlackUser[];
...
function loadChannels(dirPath: string): Map<string, SlackChannel> {
	...
	try {
		const raw = JSON.parse(readFileSync(channelsPath, "utf-8")) as SlackChannel[];
		for (const ch of raw) {
	...
	return messages.filter((msg) => {
		...
		if (options?.speakers && msg.user) {
			...
			if (!options.speakers.some((s) =>
				...
				s.toLowerCase() === msg.user!.toLowerCase()
			)) {
				return false;
			}
		}
