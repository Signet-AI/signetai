...
export function detectSchema(db: {
  ...
  try {
    const embInfo = db.prepare("PRAGMA table_info(embeddings)").all() as Array<{ name: string }>;
  ...
  if (hasMemories) {
    ...
    const hasCoreColumns = columns.includes('category') &&
 ...
 */
function migrateFromPython(db: {
  exec(sql: string): void;
  prepare(sql: string): { run(...args: unknown[]): void; all(...args: unknown[]): Record<string, unknown>[] };
}, schemaInfo: SchemaInfo): number {
  ...
  for (const row of oldMemories) {
    ...
    insert.run(
      newId, type, category, content, 1.0, importance,
      sourceId, sourceType, tagsJson, createdAt, updatedAt,
      updatedBy, lastAccessed, accessCount, pinned
    );
    migrated++;
  }
...
function migrateFromCliV1(db: {
  ...
  for (const row of oldMemories) {
    ...
    insert.run(
      id, type, null, content, 1.0, importance,
      sourceId, sourceType, tags, createdAt, updatedAt,
      updatedBy, lastAccessed, accessCount
    );
    migrated++;
  }
...
export function ensureUnifiedSchema(db: {
  exec(sql: string): void;
  prepare(sql: string): { run(...args: unknown[]): void; all(...args: unknown[]): Record<string, unknown>[]; get(...args: unknown[]): Record<string, unknown> | undefined };
}): MigrationResult {
