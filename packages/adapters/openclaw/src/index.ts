...
export function createPlugin(config: SignetConfig = {}) {
  ...
  return {
    ...
    async onSessionStart(ctx: { harness: string; sessionKey?: string }) {
      ...
      return onSessionStart(ctx.harness, { daemonUrl, sessionKey: ctx.sessionKey });
    },
    
    async onPreCompaction(ctx: { harness: string; sessionKey?: string; messageCount?: number }) {
      if (!enabled) return null;
      return onPreCompaction(ctx.harness, { daemonUrl, ...ctx });
    },
    
    async onCompactionComplete(ctx: { harness: string; summary: string; sessionKey?: string }) {
      if (!enabled) return false;
      return onCompactionComplete(ctx.harness, ctx.summary, { daemonUrl, sessionKey: ctx.sessionKey });
    },
    
    remember: (content: string, opts = {}) => remember(content, { daemonUrl, ...opts }),
