...
pub fn built_in_commands() -> Vec<SignetCommand> {
    vec![
        ...
        cmd(
            "tasks",
            "/tasks [window] [--kind <k>] [--phase <p>] [--name <n>] [--since <iso>] [--policy-denied] [--export <file>] [--jsonl]",
            "Show task telemetry (daemon-backed with filters when available); optionally export",
            CommandKind::Internal("tasks".into()),
        ),
        cmd("policy", "/policy", "Show active Forge execution policy constraints", CommandKind::Internal("policy".into())),
