...
use forge_core::{ForgeError, Message};
    ...
    pub async fn compact(
        ...
        let stream = summary_provider
            ...
            .map_err(|e| {
                ...
                ForgeError::provider(format!("Compaction LLM call failed: {e}"))
            });
        ...
        let stream = match stream {
            ...
            Err(_) if self.compaction_provider.is_some() => {
                ...
                provider
                    .complete(&summary_messages, &[], &opts)
                    .await
                    .map_err(|e2| ForgeError::provider(format!("Primary fallback also failed: {e2}")))?
            }
            Err(e) => return Err(e),
        };
        ...
        if summary_text.is_empty() {
            return Err(ForgeError::provider("Compaction produced empty summary"));
        }
