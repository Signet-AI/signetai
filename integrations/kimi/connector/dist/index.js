...
function resolvePromptSubmitTimeoutMs(raw) {
  if (!raw)
    return 5000;
  const ms = Number.parseInt(raw, 10);
  if (!Number.isFinite(ms) || ms < 1000)
...
// src/index.ts
function resolveSignetArgs() {
  if (process.platform !== "win32")
    return ["signet"];
  const entry = process.argv[1] || "";
  const signetJs = join4(entry, "..", "..", "bin", "signet.js");
  if (existsSync2(signetJs))
    return [process.execPath, signetJs];
  return ["signet"];
}
function resolveSignetMcp() {
  ...
  const entry = process.argv[1] || "";
  const mcpJs = join4(entry, "..", "..", "bin", "mcp-stdio.js");
...
function readAuthTokenEnv() {
  return readEnv2("SIGNET_API_KEY") ?? readEnv2("SIGNET_TOKEN");
}
function shellQuote(value) {
  return `'${value.replace(/'/g, "'\\''")}'`;
}
function cmdEnvQuote(value) {
  return value.replace(/[\^"&|<>]/g, "^$&");
}
function withRemoteDaemonEnv(command, remoteDaemonUrl) {
  const apiKey = readAuthTokenEnv();
  if (!remoteDaemonUrl && !apiKey)
    return command;
  if (process.platform === "win32") {
    const vars = [
...
var SIGNET_HOOK_COMMENT = "# Signet lifecycle hook (managed by signet)";
var KIMI_SESSION_START_GRACE_SECONDS = 5;
var KIMI_PROMPT_SUBMIT_GRACE_SECONDS = 2;
var SESSION_END_TIMEOUT_SECONDS = 30;
function readTimeoutEnv(name) {
  const value = process.env[name];
  return typeof value === "string" ? value.trim() : "";
}
function resolveKimiSessionStartTimeoutSeconds() {
...
function resolveKimiPromptSubmitTimeoutSeconds() {
  return Math.ceil(resolvePromptSubmitTimeoutMs(readTimeoutEnv("SIGNET_PROMPT_SUBMIT_TIMEOUT")) / 1000) + KIMI_PROMPT_SUBMIT_GRACE_SECONDS;
}
function buildKimiHookEntries(signetArgs, remoteDaemonUrl = resolveRemoteDaemonUrl()) {
  const cmd = (subcommand, kimiJson) => withRemoteDaemonEnv([...signetArgs, "hook", subcommand, "-H", "kimi", ...kimiJson ? ["--kimi-json"] : []].join(" "), remoteDaemonUrl);
  return [
    {
      event: "SessionStart",
...
var SIGNET_KIMI_HOOK_PATTERN = /\bhook\s+(?:session-start|user-prompt-submit|session-end)\s+-H\s+kimi\b/;
function isSignetKimiHookBlock(block) {
  return SIGNET_KIMI_HOOK_PATTERN.test(block.join(`
`));
}
function removeSignetKimiHookBlocks(content) {
...
function patchConfigToml(path, entries) {
...
function unpatchConfigToml(path) {
  if (!existsSync2(path))
    return false;
  const content = readFileSync2(path, "utf-8");
  if (!SIGNET_KIMI_HOOK_PATTERN.test(content))
    return false;
...
function patchMcpJson(path, mcp) {
  mkdirSync(join4(path, ".."), { recursive: true });
  const config = readMcpJson(path);
  if (config === null)
    return false;
  const existingMcp = typeof config.mcpServers === "object" && config.mcpServers !== null && !Array.isArray(config.mcpServers) ? config.mcpServers : {};
  const signetEntry = { command: mcp.command, args: [...mcp.args] };
  if (JSON.stringify(existingMcp.signet) === JSON.stringify(signetEntry))
    return false;
  config.mcpServers = { ...existingMcp, signet: signetEntry };
  atomicWriteJson(path, config);
  return true;
}
function unpatchMcpJson(path) {
  if (!existsSync2(path))
    return false;
  const config = readMcpJson(path);
  if (config === null)
    return false;
  if (typeof config.mcpServers !== "object" || config.mcpServers === null || Array.isArray(config.mcpServers) || !("signet" in config.mcpServers)) {
    return false;
  }
...
class KimiConnector extends BaseConnector {
  name = "Kimi";
  harnessId = "kimi";
  getKimiHome() {
    return readEnv2("KIMI_CODE_HOME") ?? join4(homedir(), ".kimi-code");
  }
  getConfigPath() {
    return join4(this.getKimiHome(), "config.toml");
  }
  getMcpJsonPath() {
    return join4(this.getKimiHome(), "mcp.json");
  }
  ...
  async uninstall() {
    ...
    const skillsLink = join4(this.getKimiHome(), "skills");
    if (existsSync2(skillsLink)) {
  ...
  isInstalled() {
    const configPath = this.getConfigPath();
    if (existsSync2(configPath) && SIGNET_KIMI_HOOK_PATTERN.test(readFileSync2(configPath, "utf-8"))) {
      return true;
    }
    const mcp = readMcpJson(this.getMcpJsonPath());
    return mcp !== null && typeof mcp.mcpServers === "object" && mcp.mcpServers !== null && !Array.isArray(mcp.mcpServers) && "signet" in mcp.mcpServers;
  }
}
var src_default = KimiConnector;
export {
  removeSignetKimiHookBlocks,
