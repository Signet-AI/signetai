...
The current generation of AI memory tools mostly does the same thing: store conversations in a vector database, retrieve relevant chunks when prompted, and inject them back into context. Some do it well. Some add hosted [[dashboard|dashboards]] or cloud sync. But the underlying idea is the same — save what the AI said, search it later.

That's useful. It's also not what we're building.

Signet doesn't store conversations. It doesn't compete with mem0 or supermemory or whatever the next hosted memory API is. Those tools answer the question "how do I give my AI a longer memory?" Signet answers a different question entirely.
...
The [[pipeline]] doesn't save your chat logs. It extracts knowledge from conversations — preferences, decisions, project context, relationships, tool usage patterns — and maintains that knowledge over time through decay, deduplication, and relevance scoring. If you want to understand how that extraction works at a deeper level, [[blog/knowledge-architecture|The Database Knows What You Did Last Summer]] walks through the full [[docs/knowledge-architecture|knowledge architecture]].
...
One grows unbounded and gets noisier over time. The other gets sharper. The [[pipeline]] prunes what's stale, reinforces what's confirmed, and surfaces what's relevant. Your agent doesn't just remember more — it understands better.
...
LLMs are powerful reasoning engines. But they forget everything.

Signet is the layer that remembers.
