...
In Signet, a SQLite database on your machine is the source of truth for structured memory. Memories, workspace instructions, and [Skills](https://docs.signetai.sh/skills/) stay local in formats you can inspect, back up, and version control.

Markdown files like `AGENTS.md` and `MEMORY.md` are generated *outputs* from that database. The [Daemon](https://docs.signetai.sh/daemon/) watches for changes and regenerates [harness-specific configs](https://docs.signetai.sh/harnesses/) for whatever platforms you use — Claude Code, OpenCode, Cursor, whatever comes next. The database is canonical. Everything else is a view.
...
This is what portable memory means. Not "export your data as a JSON blob and hope the next platform can parse it." Actual, live synchronization across every tool you use, powered by a daemon that watches your local database and keeps everything in sync.
...
**Verifiable** — You can read every memory your agent has stored. You can see its reasoning, its confidence scores, its decision to keep or discard a piece of information. Full observability into the [Pipeline](https://docs.signetai.sh/pipeline/) that shapes your agent's understanding of you.
...
Your agent should be yours. Your memories and secrets should be yours.
