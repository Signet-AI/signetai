...
Signet's retrieval is entirely algorithmic. It walks a [knowledge graph](/blog/knowledge-architecture/), runs FTS5 keyword search, computes vector similarity against pre-embedded memories, then applies three stages of post-fusion dampening to separate signal from noise. No agents reading through findings. No parallel orchestrators. No ensemble voting. Just data structures, indexes, and math.
...
These are not retrieval problems. They're persistence and continuity problems. They live in a layer of the stack that most memory systems don't address because they've framed the entire challenge as "find the fact."
