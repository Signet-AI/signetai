...
That is retrieval plus reasoning, one part of a persistent memory system.
...
Supermemory knew this. That was the entire point. Under their own proposed MemScore standard, ASMR scores 99% | 99% | 70,000ms | 10k tokens. Its latency and token footprint show the operating cost of that configuration.
...
Signet's retrieval is algorithmic. It walks a [knowledge graph](/blog/knowledge-architecture/), runs FTS5 keyword search, computes vector similarity against pre-embedded memories, then applies three stages of post-fusion dampening to separate signal from noise. The retrieval path uses data structures, indexes, and scoring rather than per-query agent orchestration.
...
Signet took a different path. Instead of replacing search with more intelligence at query time, we built structure that makes candidate generation bounded and coherent. The [knowledge graph](/blog/knowledge-architecture/) contains entities, aspects, attributes, constraints, and explicit dependency edges. When the system needs context about a project, it doesn't start from "search everything and hope." It can walk to the project entity, read its aspects, follow its dependencies, then let flatter retrieval layers and learned ranking handle the rest. Deterministic where possible. Bounded by design.
...
Consider what happens after you solve retrieval perfectly. Your agent can find any fact from any past conversation with 100% accuracy. Now ask: does it preserve your instructions and preferences across sessions? Does it remember *why* one architecture was chosen over another three weeks ago? Can it work in Claude Code, then continue in Cursor, then pick up in OpenCode with the same knowledge and understanding of your work?

These are persistence and continuity concerns that sit alongside retrieval. They live in the layer that carries instructions, decisions, and workspace context between sessions.

Signet addresses them because we think they're the actual hard problem.

**Durable instructions.** Signet treats workspace instructions, preferences, and behavioral guidelines as memory. These files travel across platforms with the rest of the context. The model is a guest in a persistent environment, not the owner of it. We wrote about this architecture in [Signet Is More Than a Memory API](/blog/you-think-signet-is-a-memory-system/).
...
This is a meaningful improvement over single-number reporting. Under MemScore, ASMR's latency and token footprint are visible alongside its quality scores: 99% | 99% | 70,000ms | 10k.
...
Many of the capabilities that sit on top of retrieval, durable instructions, knowledge distillation, decision tracking, and cross-platform continuity are real and shipping. Others, especially the learned scoring layer, are the convergence point the current substrate is building toward. The important claim is not that every part is perfectly mature today. It is that the architecture is aimed at a bigger problem than retrieval.
...
*Signet is open source. The [knowledge architecture](/blog/knowledge-architecture/), the desire paths traversal system, and the [extraction pipeline](https://docs.signetai.sh/pipeline/) are all part of the shipped substrate. [Local-first](/blog/why-local-first-memory/). User-owned. Retrieval requires no API keys; continuity also depends on provenance, durable instructions, and workspace context.*
