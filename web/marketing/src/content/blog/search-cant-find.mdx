...
Signet has not run MemAware. The LoCoMo benchmark validates retrieval quality (87.5% accuracy, 100% Hit@10 in our [full-stack configuration](/blog/retrieval-is-not-memory/)), but LoCoMo tests different failure modes than MemAware's cross-domain implicit context. Strong LoCoMo numbers do not predict MemAware performance. They measure different things.
...
*Signet is open source. The [knowledge architecture](/blog/knowledge-architecture/), the desire paths traversal system, and the [extraction pipeline](https://docs.signetai.sh/pipeline/) are all part of the shipped substrate. [Local-first](/blog/why-local-first-memory/). User-owned. No API keys required for retrieval.*
