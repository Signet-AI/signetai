...
NemoClaw is explicitly stateless between sessions. No memory extraction. No knowledge graph. No session persistence. No cross-session context. It secures the runtime and trusts OpenClaw to handle everything else.
...
NemoClaw secures the runtime. Signet provides memory and secrets. They're not competing layers — they're complementary. An agent running inside NemoClaw's sandbox still has no memory between sessions or predictive context. An agent running with Signet but no sandbox has memory but no security guarantees.
...
On Linux, `/home` is where your working state lives. Your dotfiles, your SSH keys, your shell configuration, your editor preferences. You can reinstall the OS, swap the kernel, change distributions entirely — and your home directory carries that state forward.
...
The agent ecosystem just declared itself modular. NVIDIA validated the layered model by building a security layer on top of an open runtime. That's the same bet we made when we built Signet as a memory and secrets layer that plugs into any harness.

Jensen is right that every company needs an OpenClaw strategy. But an OpenClaw strategy without portable memory is a strategy for agents that start over every morning. Agents that lose context mid-conversation. Agents that can't learn from yesterday's mistakes.
...
*Signet is open source and runs locally. [Install it](https://docs.signetai.sh) or [read the code](https://github.com/Signet-AI/signetai).*
