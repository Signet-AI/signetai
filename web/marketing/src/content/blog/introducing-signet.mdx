...
Models provide reasoning for a session. Signet supplies the durable workspace around that session, so context can persist across sessions and tools.
...
A tool-based memory flow asks the model to decide what to save and when to look it up. Signet records session evidence, then processes it outside the conversation into structured knowledge with provenance.

Signet uses a background distillation pipeline. A distillation engine runs outside the conversation, in the background, continuously refining raw session data into structured understanding:
...
This usage evidence gives Signet a path toward context selection that improves with the workspace, rather than relying on search alone.
...
The workspace lives on your machine: a SQLite database, markdown files, and YAML configuration. You can inspect, back up, and move these files, then connect them to supported harnesses. For the philosophy behind this choice, see [Why Local-First Memory Matters](/blog/why-local-first-memory/).
