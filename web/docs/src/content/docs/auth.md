...
Auth is optional. By default the [Daemon](/daemon/) runs in `local` mode: no tokens,
...
shorter-lived admin bearer token. See [HTTP API](/api/) for full endpoint
details.
...
[Configuration](/configuration/)). All fields are optional — omitting the section
