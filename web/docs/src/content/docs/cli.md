...
- [Setup and workspace](/cli/getting-started/): `install`, `setup`, `workspace`, `configure` (`config`), `status`, `doctor`, `dashboard` (`ui`), `sync`, and top-level daemon aliases `start`, `stop`, `restart`, `pause`, `resume`, `logs`.
...
- [Data and imports](/cli/data-portability/): `export` (`bundle`, `transcripts`), `import`, `sources`, `embed`, `migrate-schema`, `migrate-vectors`, and `repair`.
...
Run `signet <group> --help` for release-specific flags. Exact aliases include `config`, `ui`, `op`, `bw`, `inference`, `connect`, `sources remove disconnect`, and `skill uninstall remove`.