...
The memory pipeline is the daemon's bounded background runtime. It preserves and indexes evidence, maintains derived state, and exposes status to operators. “Pipeline V2” is compatibility terminology for these controls and supporting workers; it is not a per-memory LLM extraction pipeline.
...
Dreaming is the only automatic semantic writer. It selects agent-scoped episodic evidence and submits audited ontology operations. Historical extraction, decision, structural-classification, and dependency-synthesis executors are retired; historical `extract` jobs are compatibility state, not active work.
