...
export interface GraphSceneOptions {
	/** Fade a rebuilt scene in after a density change instead of popping it in. */
	fadeIn?: boolean;
}
...
export function createGraphScene(
	...
	if (options.fadeIn) {
		canvas.style.animation = "graph-density-reveal 260ms cubic-bezier(0.22, 1, 0.36, 1) both";
		labelContainer.style.animation = "graph-density-reveal 260ms cubic-bezier(0.22, 1, 0.36, 1) both";
	}
