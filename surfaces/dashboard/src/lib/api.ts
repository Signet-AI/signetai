...
async function postSourceImport(agentId: string, files: readonly File[]): Promise<ApiReadResult<SourceImportJob>> {
	...
	try {
		const res = await fetch(`${API_BASE}/api/sources/imports?agentId=${encodeURIComponent(agentId)}`, {
			...
			body: JSON.stringify({ schemaId: "signet-export", files: descriptors }),
		});
...
export const api = {
	...
	createSourceImport: (agentId: string, files: readonly File[]) => postSourceImport(agentId, files),
