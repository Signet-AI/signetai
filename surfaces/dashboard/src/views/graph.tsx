...
import { createDebouncedValue, GRAPH_DENSITY_DEBOUNCE_MS } from "@/lib/graph-density";
...
export function GraphView() {
	...
	const densityRebuildRef = useRef(false);
	const densityDebouncerRef = useRef<ReturnType<typeof createDebouncedValue<DensityRequest>> | null>(null);
	if (!densityDebouncerRef.current) {
		densityDebouncerRef.current = createDebouncedValue(GRAPH_DENSITY_DEBOUNCE_MS, (request) => {
			densityRebuildRef.current = true;
			const targetNodes = (request.pct / 100) * request.totalNodes;
			const limit = Math.max(8, Math.min(ENTITY_LIMIT_MAX, Math.round(targetNodes / request.nodesPerEntity)));
	...
	const dataSig = useMemo(() => {
		const entities = graphQuery.data?.entities ?? [];
		let h = entities.length + (sources?.length ?? 0) * 7;
	...
	useEffect(() => {
		...
		const fadeIn = densityRebuildRef.current;
		if (sceneRef.current) {
	...
	useEffect(
		() => () => {
			densityDebouncerRef.current?.cancel();
			sceneRef.current?.dispose();
			sceneRef.current = null;
		},
		[],
	...
	const onDensityChange = (pct: number) => {
		setSliderPct(pct);
		densityDebouncerRef.current?.schedule({ pct, totalNodes, nodesPerEntity });
	};
