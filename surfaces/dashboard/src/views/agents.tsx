...
export function AgentsView() {
	...
	const confirmEdit = async () => {
		...
		setSaving(false);
		if (result.error || !result.data) {
			setError(result.error ?? "Agent update failed");
			return;
		}
		setPending(null);
		setEditing(null);
	...
	return (
		<section className="flex min-h-0 flex-1 flex-col gap-5 overflow-auto" aria-labelledby="agents-title">
			<header>
				<p className="font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">System</p>
				<h1 id="agents-title" className="text-xl font-semibold tracking-tight">
					Agents
				</h1>
				<p className="mt-1 text-sm text-muted-foreground">Daemon-owned memory policy and group access.</p>
			</header>
			{error && (
			...
			{confirmation && (
				<div role="status" className="rounded-md border border-border bg-muted/50 p-3 text-sm">
					Saved <strong>{confirmation.name}</strong>: daemon resolved scope to{" "}
					<ScopeBadge>{confirmation.effective_scope ?? "unknown"}</ScopeBadge>
					{confirmation.policy_group ? (
						<>
							{" "}
							in <ScopeBadge>{confirmation.policy_group}</ScopeBadge>
						</>
					) : null}
				</div>
			)}
			...
			{agents?.length === 0 && (
				<div className="rounded-md border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
					No agents registered with the daemon.
				</div>
			)}
								...
								{editing === agent.name ? (
									<div className="flex w-full flex-wrap items-center gap-2 border-t border-border pt-3">
										<select
											...
											value={draft.policy}
											onChange={(e) => setDraft({ ...draft, policy: e.target.value as Policy })}
										...
										<button
											type="button"
											disabled={saving}
											onClick={() => setPending({ agent, draft })}
											className="rounded bg-primary px-3 py-1 text-sm text-primary-foreground"
										>
											Confirm
										</button>
										<button type="button" onClick={() => setEditing(null)} className="px-2 py-1 text-sm underline">
											Cancel
										</button>
									</div>
								) : (
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Confirm agent scope change</DialogTitle>
						<DialogDescription>
							Update <strong>{pending?.agent.name}</strong> to <ScopeBadge>{pending?.draft.policy ?? ""}</ScopeBadge>
							...
							? The daemon will resolve the effective scope after saving.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<button
