...
describe("version-sync", () => {
	...
	test("keeps the Kimi connector package aligned with the release version", () => {
		const root = JSON.parse(readFileSync(join(import.meta.dir, "..", "package.json"), "utf8")) as {
			version?: unknown;
		};
		const kimi = JSON.parse(
			readFileSync(join(import.meta.dir, "..", "integrations/kimi/connector/package.json"), "utf8"),
		) as { version?: unknown };

		expect(typeof root.version).toBe("string");
		expect(kimi.version).toBe(root.version);
	});
