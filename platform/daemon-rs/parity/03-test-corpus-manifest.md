...
| `platform/daemon/src/native-memory-sources.test.ts` | Native Codex/Claude memory source discovery, symlink rejection, dedupe, and purge | needs-port | NONE (port note: TS side had a bug where source-sync callers omitted embeddingConfig/fetchEmbedding, so content never got chunked/embedded; fixed via native-memory-sources.ts resolveEmbeddingBridgeOptions — wire the Rust port's equivalent options the same way from day one) |
...
| `platform/daemon/src/obsidian-source-embeddings.test.ts` | Obsidian markdown chunking, source chunk embeddings, purge, sqlite-vec mirror, and recall | needs-port | NONE (generic document chunking only: platform/daemon-rs/crates/signet-pipeline/src/document.rs; port note: see native-memory-sources.test.ts row re: embeddingConfig/fetchEmbedding wiring bug) |
