...
export function indexDiscordSourceStructure(input: IndexDiscordSourceStructureInput): void {
	...
	getDbAccessor().withWriteTx((db) => {
		...
		for (const participant of input.participants) {
			const participantEntityId = idFor(input.agentId, input.sourceId, "participant", participant);
			upsertEntity(db, {
				id: participantEntityId,
				name: participant,
				canonicalName: `discord:user:${participant}`,
				entityType: "source_document_reference",
				agentId: input.agentId,
				sourceId: input.sourceId,
				sourcePath: `discord:user:${participant}`,
				now,
			});
			upsertDependency(db, {
				...
				strength: 0.7,
				confidence: 1,
				reason: requireDependencyReason("related_to", `Participant ${participant} in ${threadLabel}`),
				sourceId: input.sourceId,
				now,
			});
		}
