...
export async function syncDiscordSource(
	...
	for (const guildId of settings.guildIds) {
		...
		for (const channel of filteredChannels) {
			...
			try {
				const msgResult = await fetchChannelMessages(config, channel.id, settings.maxMessagesPerChannel);
...
async function syncThreads(
	...
	const activeResult = await fetchActiveThreads(config, parentChannelId);
	const archivedResult = await fetchPublicArchivedThreads(config, parentChannelId, 50);
	const allThreads = [...activeResult.data, ...archivedResult.data];
	...
	for (const thread of uniqueThreads) {
		try {
			...
			const msgResult = await fetchChannelMessages(config, thread.id, settings.maxMessagesPerChannel);
			if (msgResult.data.length > 0) {
				const participants = extractParticipants(msgResult.data);
				indexDiscordSourceStructure({
					agentId,
...
function extractParticipants(messages: readonly DiscordMessage[]): string[] {
	const seen = new Set<string>();
