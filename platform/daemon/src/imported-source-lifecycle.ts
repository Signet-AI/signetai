...
import type { DbOwnerImportedSourceUnsupported } from "./db-owner-protocol";
 ...
 */
export async function markImportedSourceUnsupported(
	input: MarkImportedSourceUnsupportedInput,
): Promise<MarkImportedSourceUnsupportedResult> {
	const { dbOwnerImportedSourceUnsupported } = await import("./db-owner-runtime");
	return await dbOwnerImportedSourceUnsupported(input, {
		operation: "imported-source.unsupported",
		deadlineMs: 30_000,
	});
}

export function markImportedSourceUnsupportedInTx(
	input: MarkImportedSourceUnsupportedInput | DbOwnerImportedSourceUnsupported,
