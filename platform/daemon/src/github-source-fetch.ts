...
export async function fetchIssueComments(config: GitHubFetchConfig, issueNumber: number): Promise<GitHubComment[]> {
	...
	while (comments.length < MAX_COMMENTS_PER_RESOURCE) {
		...
		const response = await githubRequest(url, config.token);
		if (response.status !== 200) throw new Error(`Issue comments fetch failed for #${issueNumber}: ${response.status}`);
...
export async function fetchPullRequestComments(
	...
	while (comments.length < MAX_COMMENTS_PER_RESOURCE) {
		...
		const response = await githubRequest(url, config.token);
		if (response.status !== 200)
			throw new Error(`Pull request comments fetch failed for #${pullNumber}: ${response.status}`);
		const batch = response.body as GitHubComment[];
		comments.push(...batch);
		if (batch.length < PER_PAGE) break;
		page++;
...
export async function fetchDiscussionComments(
	...
	while (true) {
		...
		const response = await githubRequest(GRAPHQL_URL, config.token, "POST", { query, variables });
		if (response.status !== 200)
			throw new Error(`Discussion comments fetch failed for #${discussionNumber}: ${response.status}`);
		const data = response.body as {
			data?: {
				repository?: {
					discussion?: {
...
async function fetchTreeDocs(
	...
	if (treeResponse.status !== 200) {
		errors.push({
			message: `Tree fetch failed for ${dir}: ${treeResponse.status}`,
			retryable: treeResponse.status >= 500,
		});
