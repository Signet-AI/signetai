	...
	return {
		run: async <Result>(operation: ImportStoreOperation): Promise<Result> => {
			...
			if (operation.operation === "control") {
				const control = p.control as "start" | "pause" | "resume" | "retry" | "cancel";
				if (p.apply === true) {
					return (await dbOwnerTransaction(
						[
							{
		...
		[
			...
			...(input.files ?? []).map((file, ordinal) => ({
				...
				params: [
					file.id,
					input.jobId,
					`reserved:${input.jobId}:${file.id}`,
					input.agentId,
					ordinal,
					file.name,
					`imports/transcripts/${input.jobId}/${file.id}/source.jsonl`,
				],
				result: "run" as const,
			})),
		],
