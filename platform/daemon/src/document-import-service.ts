...
import { IMPORT_MAX_BATCH_BYTES, IMPORT_MAX_FILE_BYTES, normalizeImportedFile } from "./import-normalizer";
...
export type ImportDuplicateMode = "skip" | "replace" | "reimport";
...
export interface DocumentImportInput {
	...
	readonly duplicateMode?: ImportDuplicateMode;
...
export async function importDocument(input: DocumentImportInput): Promise<DocumentImportResult> {
	...
	if (/\.jsonl$/i.test(input.fileName))
		return {
			status: {
				fileName: input.fileName,
				status: "failed",
				error: "Transcript JSONL must use the canonical transcript importer",
			},
			persistedBytes: 0,
		};

	const normalized = await normalizeImportedFile(input.fileName, input.bytes, input.contentType ?? "");
	if (normalized.ok === false)
		return { status: { fileName: input.fileName, status: "failed", error: normalized.error }, persistedBytes: 0 };
	const persistedBytes = persistedImportBytes(normalized.value);
	if (persistedBytes > (input.maxPersistedBytes ?? IMPORT_MAX_BATCH_BYTES))
		return {
			status: {
				fileName: input.fileName,
				status: "failed",
	...
	const agentsDir = input.workspaceRoot ?? process.env.SIGNET_PATH;
	const agentId = input.agentId ?? resolveDaemonAgentId();
	const duplicateMode = input.duplicateMode ?? "skip";
	...
	try {
		...
		if (replacedSource !== undefined) {
			...
			if (removed.ok === false) throw new Error(`Replaced dashboard import config cleanup failed: ${removed.error}`);
		}
		...
		if (added.created)
			try {
				await runWriteTxAsync(getDbAccessor(), (db) =>
					purgeSourceOwnedRowsInTx(db, { sourceId: added.source.id, agentId }),
				);
				const removed = removeSourceIfGeneration(added.source.id, added.source.generation, agentsDir);
