...
export function purgeGitHubResourceStructure(input: PurgeGitHubResourceStructureInput): number {
	...
	return getDbAccessor().withWriteTx((db) => {
		...
		if (!entity) return 0;
		const attrs = db
			.prepare("DELETE FROM entity_attributes WHERE entity_id = ? AND agent_id = ?")
			.run(entity.id, input.agentId).changes;
		const deps = db
			.prepare("DELETE FROM entity_dependencies WHERE (source_entity_id = ? OR target_entity_id = ?) AND agent_id = ?")
			.run(entity.id, entity.id, input.agentId).changes;
		const entities = db
			.prepare("DELETE FROM entities WHERE id = ? AND agent_id = ?")
			.run(entity.id, input.agentId).changes;
