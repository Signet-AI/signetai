...
export interface OntologyProposalEvidenceItem {
	readonly kind: "provided_quote" | "session_transcript" | "memory_artifact" | "unresolved";
...
function readEvidenceRef(value: unknown): EvidenceRef | null {
	if (typeof value === "string") {
		const trimmed = value.trim();
		return trimmed.length > 0
			? { sourceKind: null, sourceId: trimmed, sourcePath: null, quote: null, reference: value }
			: null;
	}
...
function proposalEvidenceRefs(proposal: OntologyProposal): EvidenceRef[] {
	const refs = proposal.evidence.map(readEvidenceRef).filter((ref): ref is EvidenceRef => ref !== null);
	...
	return refs.filter((ref) => {
		const key = [ref.sourceKind ?? "", ref.sourceId ?? "", ref.sourcePath ?? "", ref.quote ?? ""].join("\0");
...
function readMemoryArtifactEvidence(db: ReadDb, agentId: string, ref: EvidenceRef): MemoryArtifactEvidenceRow | null {
...
function sourceLooksLikeTranscript(ref: EvidenceRef): boolean {
	return (
...
function resolveEvidenceRef(db: ReadDb, agentId: string, ref: EvidenceRef): OntologyProposalEvidenceItem {
	...
	return {
		...
		sourceId: ref.sourceId,
		sourcePath: ref.sourcePath,
		label: ref.sourcePath ?? ref.sourceId ?? "unknown evidence",
		excerpt: "",
		reference: ref.reference,
	};
}
...
export function getOntologyProposalEvidence(
	...
	const items = accessor.withReadDb((db) =>
		proposalEvidenceRefs(proposal).map((ref) => resolveEvidenceRef(db, agentId, ref)),
	);
