...
fn execute_operation(
    ...
    match operation {
        ...
        Operation::SessionCandidatesAssemble { agent_id, workspace_id, session_key, token_budget } => {
            ...
            let mut stmt = connection.prepare("SELECT sm.memory_id,m.content,sm.source,sm.effective_score,sm.final_score,sm.rank,sm.was_injected,sm.path_json,sm.relevance_score,sm.fts_hit_count,sm.entity_slot,sm.aspect_slot,sm.is_constraint,sm.structural_density,sm.predictor_score,sm.agent_preference FROM session_memories sm JOIN memories m ON m.id=sm.memory_id AND COALESCE(m.agent_id,'default')=sm.agent_id WHERE sm.session_key=? AND sm.agent_id=? AND sm.workspace_id=? AND sm.was_injected=1 AND COALESCE(m.is_deleted,0)=0 AND COALESCE(m.deleted,0)=0 AND m.superseded_by IS NULL AND COALESCE(json_extract(m.metadata,'$.supersededBy'),json_extract(m.metadata,'$.superseded_by')) IS NULL AND m.stale_at IS NULL AND COALESCE(json_extract(m.metadata,'$.staleAt'),json_extract(m.metadata,'$.stale_at')) IS NULL AND COALESCE(m.source_type,'') != 'aggregate-recall' ORDER BY sm.rank ASC")?;
            let mut used = 0i64;
