    ...
    pub fn recall(&self, agent: &str, query: &str) -> Result<Vec<Memory>, CoreError> {
        ...
        self.call(move |connection| {
            let mut statement = connection.prepare(
                "SELECT id, agent_id, content, metadata, deleted, created_at, updated_at, source_id, source_type, source_path, runtime_path, idempotency_key, memory_kind
                 FROM memories
                 WHERE COALESCE(agent_id, 'default') = ? AND is_deleted = 0 AND content LIKE ? ESCAPE '\\'
                 ORDER BY rowid DESC LIMIT 1000",
            )?;
            let escaped_query = query.replace('\\', "\\\\").replace('%', "\\%").replace('_', "\\_");
            let rows = statement.query_map(params![agent, format!("%{escaped_query}%")], memory_row)?;
            Ok(rows.collect::<Result<Vec<_>, _>>()?)
        })
    }
    ...
    match operation {
        ...
        Operation::ReflectionAnswer {
            ...
            if answer.trim().is_empty() {
                return Err(CoreError::InvalidInput("answer is required".into()));
            }
            if answer.trim().chars().count() > 10_000 {
                return Err(CoreError::InvalidInput(
                    "answer exceeds 10000 characters".into(),
                ));
            }
            let tx = connection.transaction()?;
            ...
            if changed == 0 {
                let exists: i64 = tx.query_row(
                    "SELECT count(*) FROM daily_reflections WHERE id=? AND agent_id=?",
                    params![id, agent_id],
                    |r| r.get(0),
                )?;
                if exists == 0 {
                ...
                return Err(CoreError::InvalidInput("Already answered".into()));
            }
            let metadata = serde_json::to_string(
                &json!({"type":"reflection","sourceType":"reflection-answer","sourceId":id,"contentHash":format!("reflection-a-{id}"),"why":"daily-reflection-answer"}),
            )?;
            ...
            record_history(&tx, &memory_id, &agent_id, "remember", None)?;
            tx.commit()?;
        ...
        Operation::SessionCandidatesAssemble {
            ...
            let session_key = required_id(&session_key)?;
            if token_budget == 0 {
                return Ok(json!({"items": [], "tokens": 0, "tokenBudget": 0}));
            }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct OntologyProposalEvidenceRequest {
    pub agent_id: String,
    pub id: String,
}
...
fn migrate(connection: &mut Connection) -> Result<(), CoreError> {
    ...
    transaction.execute("UPDATE memories SET source_id=COALESCE(NULLIF(source_id,''),CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.sourceId') END,CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.source_id') END), source_type=COALESCE(NULLIF(source_type,''),CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.sourceType') END,CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.source_type') END), source_path=COALESCE(NULLIF(source_path,''),CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.sourcePath') END,CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.source_path') END), runtime_path=COALESCE(NULLIF(runtime_path,''),CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.runtimePath') END,CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.runtime_path') END), idempotency_key=COALESCE(NULLIF(idempotency_key,''),CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.idempotencyKey') END,CASE WHEN json_valid(metadata) THEN json_extract(metadata,'$.idempotency_key') END) WHERE metadata IS NOT NULL", [])?;
