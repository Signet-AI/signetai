...
fn memory_search_returns_matches_when_legacy_metadata_is_malformed() {
    ...
    core.submit(Operation::Remember {
        agent_id: "agent".into(),
        content: "valid legacy needle".into(),
        metadata: serde_json::json!({}),
    })
    .unwrap();
    drop(core);
    ...
    db.execute(
        ...
        ("{malformed", "2025-01-01T00:00:00Z", "legacy malformed needle"),
    )
    .unwrap();
    drop(db);
    ...
    assert_eq!(ids.len(), 2);
    assert!(ids.iter().any(|row| row["content"] == "valid legacy needle"));
    assert!(ids.iter().any(|row| row["content"] == "legacy malformed needle"));
}
